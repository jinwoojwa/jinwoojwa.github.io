...
매번 `for`문으로 합을 구하면 최악의 경우 $10^5 \times 10^5=10^{10}$ 번의 연산이 필요하다.

1초 내에 해결하기 위해서는 $O(N+M)$ 의 구간 합 알고리즘을 사용해야 한다.
...
- 구간 합 알고리즘을 통해 $(N \times M)$ 의 반복 연산을 $O(N+M)$ 으로 줄일 수 있다.
